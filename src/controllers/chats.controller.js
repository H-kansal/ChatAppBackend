...
export const createNewChat=asyncHandler(async(req,res)=>{
    ...
    const chat=await Chat.find({isGroupChat: false,users: { $all: [userId, adminId]}}).populate("users", "-password").populate("groupAdmin", "-password").populate("latestMessage");
    
    if(chat.length>0){
        return res.status(200).json(new ApiResponse(200,chat[0],"chat already exists"));
    }
